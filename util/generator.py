...
from asn1ate.sema import (DefinedType, ValueAssignment, TypeAssignment, TaggedType, SimpleType, BitStringType,
                          ValueListType, SequenceType, SetType, ChoiceType, SequenceOfType, SetOfType, ComponentType,
                          dependency_sort, ExtensionMarker, TagImplicitness, NamedType)
from quick_der.main import QuickDERgeneric, tosym
...
class QuickDER2gijs(QuickDERgeneric):
        ...
        for assigncompos in dependency_sort(self.semamod.assignments):
            for assign in assigncompos:
                ...
                if tnm in self.psub_funmap:
                    logger.debug('Recursive call for', tnm)
                    self.psub_funmap[tnm](assign, None, None, True)
                    logger.debug('Recursion done for', tnm)
                else:
    ...
    def generate_psub_node(self, node, tp, fld, prim):
        tnm = type(node)
        logger.debug('generate_psub_node() CALLED ON', tnm)
        if tnm in self.psub_funmap:
    ...
    def psubTypeAssignment(self, node, tp, fld, prim):
        ...
        logger.debug('SUBTRIPLES =', subquads)
        if subquads:
    ...
    def psubDefinedType(self, node, tp, fld, prim):
        logger.debug('DefinedType type:', node.type_name, '::', type(node.type_name))
        modnm = node.module_ref
        logger.debug('AFTER modnm = node.module_ref', modnm)
        if not modnm and self.semamod.imports:
            syms = self.semamod.imports.imports
            logger.debug('SYMS.KEYS() =', syms.keys())
            for mod in syms.keys():
                if node.type_name in syms[mod]:
                    modnm = str(mod).lower()
                    logger.debug('AFTER modnm = str(mod).lower ()', modnm)
                    break
        if modnm is None:
            # NOT_IN_GENERAL# modnm = node.module_ref
            modnm = self.unit.lower()
            logger.debug('AFTER modnm = self.unit.lower ()', modnm)
            logger.debug('MODNM =', modnm, '::', type(modnm))
            logger.debug('Referenced module:', modnm, '::', type(modnm))
            logger.debug('Searching case-insensitively in:', self.refmods.keys())
        ...
        logger.debug('References:', thetype, '::', type(thetype))
        popunit = self.unit
        ...
        self.unit = popunit
        logger.debug('SUBTUPLES =', subtuples)
        return subtuples
    ...
    def psubConstructedType(self, node, tp, fld, prim):
        ...
        for comp in node.components:
            if isinstance(comp, ExtensionMarker):
                continue
            subfld = tosym(comp.identifier)
            logger.debug('subfld is ', subfld)
            logger.debug('Generating PSUB node for %s', comp.type_decl.type_name)
            subquads = self.generate_psub_node(comp.type_decl, tp, subfld, prim)
            logger.debug('Generated  PSUB node for %s', comp.type_decl.type_name)
            logger.debug('quads are %s', subquads)
            if fld == '0':
                subtp = tp
            else:
                subtp = tp + '_' + fld
            ...
            for (idx, esz, pck, psb) in subquads:
                logger.debug('DEALING WITH', pck)
                if str(idx) == '0':
                    idx = ofs
                else:
                    idx = ofs + ' \\\n\t\t+ ' + str(idx)
                compquads.append((idx, esz, pck, psb))
        logger.debug('psubConstructedType() RETURNS COMPONENT TRIPLES', compquads)
        return compquads
    ...
    def psubRepeatingStructureType(self, node, tp, fld, prim):
        ...
        if prim:
            ...
            logger.debug('FIRST STEP OF psubRepeatingStructureType()')
            self.comma()
            ...
            logger.debug('SECOND STEP OF psubRepeatingStructureType()')
            logger.debug('PROVIDED', tp)
            subquads = self.generate_psub_node(elem_type, tp, fld, False)
            # 3. produce triple structure definition for the nested field
            logger.debug('THIRD STEP OF psubRepeatingStructureType()')
            ...
            logger.debug('FOURTH STEP OF psubRepeatingStructureType()')
        nam = self.unit + '_' + tp
        idx = '0'
